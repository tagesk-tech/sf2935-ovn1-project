...
Before training, deterministic tests check data seeding and tensor shapes. For the hand-worked case $Z=(1,-2)$, $X_1=(3,1)$, $t=0.25$, and $\smin=0.1$, the implementation returns $X_t=(1.525,-1.3)$ and target velocity $(2.1,2.8)$. A constant-vector-field test also verifies the midpoint integrator exactly.
...
The submitted program regenerates all fits, metrics, and figures with one command. It records the full configuration and library versions. The complete run was repeated on CPU and produced identical loss sequences. Checkpoints are excluded from version control because they are regenerated by the experiment command; all report figures and CSV/JSON evidence are tracked.
...
\noindent\textbf{Important before signing.} This is an AI-assisted draft. Both group members must independently verify every equation, rerun and understand the code, revise the analysis into their own substantiated account, and ensure the disclosure below is complete. Do not sign until every declaration is true.
...
\begin{tabularx}{\linewidth}{@{}p{0.34\linewidth}X@{}}
  ...
  \midrule
  OpenAI Codex, GPT-5 family (2026-09-22; \todo{insert exact model/version shown in the app}) & Reference implementation, debugging, experiment orchestration, mathematical checking, literature summary, LaTeX drafting, and layout verification. \\
  \bottomrule
\end{tabularx}
...
\begin{tabularx}{\linewidth}{@{}p{0.43\linewidth}X@{}}
  ...
  \midrule
  $\boxtimes$ Writing or debugging code & Drafted the three-module reference implementation, deterministic checks, plotting, metrics, and experiment runner; the group must verify and modify every submitted line. \\
...
\begin{tabularx}{\linewidth}{@{}p{0.20\linewidth}p{0.13\linewidth}X p{0.18\linewidth}@{}}
  ...
  \midrule
  Tage Sköldefors & \todo{ID} & \todo{Describe actual contribution after review and revision.} & \\
  \addlinespace[1.2em]
  \todo{Partner} & \todo{ID} & \todo{Describe actual contribution.} & \\
  \addlinespace[1.2em]
  \bottomrule
\end{tabularx}
